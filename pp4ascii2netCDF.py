...
def main():
    ...
    fileList = sorted(glob.glob("pp4data/byDayUTC/*"))   

    for f in fileList:
        currentFile = open(f, 'r')
        dsetFilename = "pp4Outfiles/pp04_" + currentFile.name[17:21] + "_" + currentFile.name[21:23] + "_" + currentFile.name[23:25] + ".nc"
        ...
        for l in wholeFile:
            ...
            date = datetime.datetime(int(splitLine[6][7:11]), monthToNumber(splitLine[6][3:6]), int(splitLine[6][0:2]), int(splitLine[6][12:14]), int(splitLine[6][15:17]), int(splitLine[6][18:20]), 0, pytz.timezone('US/Hawaii'))
            tz = pytz.utc
       ...
       # temperature = temperature.reshape(18,1,1,1)
        ...
        toWriteDataset.variables["lat"][:] = 21.2909
        toWriteDataset.variables["lon"][:] = -157.8606
